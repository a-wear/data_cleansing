...
from run_cleandb import run_cleandb
...
if __name__ == '__main__':
    ...
    # Check if all the parameters are in the configuration file
    ...
    if datasets != '':
        ...
        for i, dataset in enumerate(datasets):
            for j in range(0, len(config['dataset'])):
                if dataset == config['dataset'][j]['name']:
                    ...
                    if algorithm == "CLEAN":
                        run_cleandb(dataset_name=dataset, path_config=config['path'], dataset_config=config['dataset'][j], model_config=config['model_config'])
                    else:
                        print(misc.log_msg("ERROR",
        ...
        for dataset in config['dataset']:
            ...
            if algorithm == "CLEAN":
                run_cleandb(dataset_name=dataset, path_config=config['path'], dataset_config=dataset, model_config=config['model_config'])
            else:
                print(misc.log_msg("ERROR",
                                   "Algorithm not available"
                                   "{CNN-ELM|ELM}"))
